...
//
//  Copyright (c) 2013-2016 MobileIron. All rights reserved.
//
...
typedef enum {
    ACPASTEBOARDPOLICY_UNAUTHORIZED =  0, // The application cannot write data to the pasteboard. The AppConnect library
                                          // enforces this status and ensures that the app cannot modify the pasteboard contents.
    ACPASTEBOARDPOLICY_AUTHORIZED   =  1, // The application may write data to the pasteboard which gets shared among
                                          // all apps (Both AppConnect and non-AppConnect apps can read this data).
    ACPASTEBOARDPOLICY_SECURECOPY   =  2, // The application may write data to the general pasteboard which is shared
...
// pasteboard policy is ACPASTEBOARDPOLICY_SECURECOPY. When the secure services are available, the app will be able
// write data to the pasteboard securely so that only authorized AppConnect apps can read that data. Also an app can read
// secure pasteboard data written by another app only when the secure services are available. When secure services are
// not available, writing to the general pasteboard will fail and reading from it will read unsecured content, if any.
...
// Notification that the application is becoming active again due to AppConnect event such as a flip from MobileIron client.
//
// This notification only occurs after an AppConnect event such as a flip to the MobileIron client caused the app to
// resign active. Not every such event is followed by this notification. For instance, this notification does not occur
// if the user presses the home button to cancel the operation, and then returns to the app.
